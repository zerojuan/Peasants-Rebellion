(function(module){
	module.exports = {
	getPossibleMoves : function(board, piece){
		var pos = {
			row: piece.row,
			col: piece.col
		};
		var endGame = this.endGameCheck(board, piece.type, pos,
			pos, piece.color);
		...
		for(var i in allPossibleMoves){
			...
			if(move.from.row == pos.row && move.from.col == pos.col){
				...
				nextBoard[move.to.row][move.to.col] = move.color+''+move.type;
				var enemyMoves = this._getAllPossibleMoves(nextBoard, (move.color == 'W' ? 'B' : 'W'), false);
				var checkers = this.getChecker(nextBoard, enemyMoves, move.color);
				if(!checkers){
					myPossibleMoves.push(move);	
				}				
			}
		}
